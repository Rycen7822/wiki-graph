...
def run_wiki_status(args: argparse.Namespace) -> dict[str, Any]:
    command = [sys.executable, "-m", "ops.batch_wiki_integration", "status", "--root", str(args.root), "--state-dir", str(args.state_dir), "--reason", "threshold"]
    if args.threshold is not None:
        command.extend(["--threshold", str(args.threshold)])
    result = run_json(command, cwd=args.workdir, timeout=args.timeout)
...
def run_native_refresh_status(args: argparse.Namespace) -> dict[str, Any]:
    command = [sys.executable, "-m", "ops.batch_native_refresh", "status", "--root", str(args.root), "--state-dir", str(args.state_dir), "--workdir", str(args.workdir)]
    result = run_json(command, cwd=args.workdir, timeout=args.timeout)
